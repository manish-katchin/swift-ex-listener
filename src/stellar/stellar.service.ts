...
export class StellarService {
  ...
  async getCurrentLedger() {
    const ledger = await this.server.ledgers()
      .order("desc")     
      .limit(1)
      .call();
  ...
  async sendNotification(address, tokenType, value, altText, from, txHash, type) {
    let network = "STR";
    ...
    const fcmToken: string | null = await this.redisService.hGet(process.env.STELLAR_REDIS_KEY as string, address);
    if (fcmToken) {
  ...
  async parseStellarEffect(effect) {
    ...
    if (effect.type === "account_credited" && await this.redisService.isKeyExist(
      process.env.STELLAR_REDIS_KEY as string,
      effect.account
    )) {
      const chain: string = effect.asset_type === 'native' ? 'XLM' : effect.asset_code;
      this.sendNotification(effect.account, chain, effect.amount, "Received: ", `${effect.account.slice(0, 4)}...${effect.account.slice(-4)}`, effect.transaction_hash, "trf");
    }

    if (effect.type === "trade" && await this.redisService.isKeyExist(
      process.env.STELLAR_REDIS_KEY as string,
      effect.account
    )) {
      const soldAsset: string = effect.sold_asset_type === 'native' ? 'XLM' : effect.sold_asset_code;
      const boughtAsset: string = effect.bought_asset_type === 'native' ? 'XLM' : effect.bought_asset_code;
      let altText = `Bought: ${effect.bought_amount} ${boughtAsset}, Sold:`;
      this.sendNotification(effect.account, soldAsset, effect.sold_amount, altText, `Orderbook`, effect.transaction_hash, "swap");
    }
  }
  ...
  async fetchLedgerEffects(ledgerNumber) {
    let page = await this.server.effects().forLedger(ledgerNumber).limit(200).call();
